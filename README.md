...
- **Code generation**: emit the smart cursor-init block before a trailing `END PROGRAM`; wrap over-long generated `CALL` argument lines at column 72; keep `yylineno` in step across COBOL literal continuation lines; bind each query id to its `ws_query_list` position (fixes a `std::out_of_range` abort); emit the cursor-init flags with `VALUE SPACE` so the DECLARE/OPEN gate works under dialects that zero-fill WORKING-STORAGE without a `VALUE` clause (e.g. GnuCOBOL `-std=ibm`); and define an IBM Db2-compatible `SQL-INIT-FLAG` symbol when the source uses it.
